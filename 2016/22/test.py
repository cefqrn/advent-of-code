...
target = Position(0, 0)
possibilities = [Possibility(h(empty, goal), 0, empty, goal)]
seen = {(empty, goal): 0}
while possibilities:
  ...
  for neighbor in empty.neighbors:
    ...
    if (previous_cost := seen.get((neighbor, new_goal))) is not None and previous_cost <= cost: continue
    seen[(neighbor, new_goal)] = cost
