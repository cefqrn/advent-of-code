...
def solve(data):
    ...
    while left:
        curr = left.popleft()
        while left:
            merging = left.popleft()
            if merging.start < curr.stop:
                curr = range(curr.start, merging.stop)
            else:
                left.appendleft(merging)
                break

        print(curr)

        result += curr.stop - curr.start
