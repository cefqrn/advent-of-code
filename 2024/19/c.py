...
patterns = patterns.encode().split(b", ")


from functools import cache

@cache
def can_create(design):
    if not len(design):
        return 1
    ...
    for pattern in patterns:
        ...
        if design[:pattern_length] == pattern:
            s += can_create(design[pattern_length:])

    return s

for design in b:
    c = can_create(memoryview(design.encode()))
    p1 += bool(c)
    p2 += c
